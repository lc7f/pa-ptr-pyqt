...
class Window(QMainWindow, Ui_MainWindow):
    ...
    def abre_manual(self):
        print('abre manual')
        os.startfile('resources\\Manual.pdf')
    ...
    def carrega_dados_pa(self):
        ...
        try:
            etapas = pd.read_excel(arquivo_parametros, engine="odf")
        except FileNotFoundError:
    ...
    def identifica_perturbacoes(self, df_pa, tempo):
        ...
        for i in range(0, 15):
            if tempo + 9 < df_pa['Time'].iloc[-1]:
                intra = df_pa[(df_pa['Time'] > tempo + 1.5) & (df_pa['Time'] < tempo + 3.5)]
                inter = df_pa[(df_pa['Time'] > tempo + 7) & (df_pa['Time'] < tempo + 9)]
